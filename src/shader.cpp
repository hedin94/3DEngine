...
void Shader::update(Transform* transform, RenderingEngine* engine, Material* material)
{
	...
	{
	    if (uniform.second.insideStruct) continue;
	    ...
	    else
		{
		    ...
		    else if(type == "float" && material->floatExists(name)) {
...
void Shader::addUniform(const std::string& uniformName, const std::string& uniformType, const bool& insideStruct)
{
    int location = glGetUniformLocation(m_program, uniformName.c_str());  
    assert(location != GL_INVALID_VALUE);
  
    m_uniforms.emplace(uniformName, UniformData(location, uniformType, insideStruct));
}

void Shader::addUniformWithCheck(const std::string& type, const std::string& name, uniform_structs structs)
{
    addUniform(name, type, name.find('.') != std::string::npos);
...
void Shader::setUniformV(const std::string& name, float* value)
{
    glUniform1fv(m_uniforms.at(name).location, 1, value);
}
...
void Shader::setUniform(const std::string& name, BaseLight* baseLight)
{
    ...
    setUniformV(name + ".intensity", &baseLight->intensity);
}

void Shader::setUniform(const std::string& name, DirectionalLight* directionalLight)
{
    setUniform(name + ".base", dynamic_cast<BaseLight*>(directionalLight));
...
{
    setUniform(name + ".base", dynamic_cast<BaseLight*>(pointLight));
    setUniformV(name + ".atten.constant", &(pointLight->atten.constant));
    setUniformV(name + ".atten.linear", &(pointLight->atten.linear));
    setUniformV(name + ".atten.quadratic", &(pointLight->atten.quadratic));
    setUniform(name + ".position", pointLight->getTransform()->getPos());
    setUniformV(name + ".range", &(pointLight->range));
}

void Shader::setUniform(const std::string& name, SpotLight* spotLight)
{
    setUniform(name + ".pointLight", dynamic_cast<PointLight*>(spotLight));
    setUniform(name + ".direction", spotLight->getTransform()->getForward());
    setUniformV(name + ".cutoff", &(spotLight->cutoff));
}
    ...
    while(std::getline(input, line))
	{
	    if(line.substr(0, 6) == "struct")
		{
		    ...
		    while(std::getline(input, line) && line != "};")
			{
			    ...
			    if (componentName.at(componentName.size() - 2) == ']')
				componentName = componentName.substr(0, componentName.size()-4);
			    else
